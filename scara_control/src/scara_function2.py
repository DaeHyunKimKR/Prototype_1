...
class Scara():
    ...
    def __init__(self):
        # Set serial port
        self.port = serial.Serial(port='/dev/SCARA2', baudrate=57600, timeout=0, parity=serial.PARITY_NONE,
        stopbits=serial.STOPBITS_ONE,bytesize=serial.EIGHTBITS)
        self.index_setting([4,5,6])
    ...
    def inverse_calc_old(self, x, y):
        l1 = 350
        l2 = l1
        ctheta2 = (x**2 + y**2 - l1**2 - l2**2)/(2 * l1 * l2)
        theta2 = math.acos(ctheta2)
        theta1 = math.atan2(x, y) - math.atan2((l2*math.sin(theta2)), (l1 + l2*ctheta2))
    ...
    def check_workspace(self, x, y):
        ...
        p1 = [0, 0]
        p2 = [-700, (700*math.tan(math.radians(40)))]
        p3 = [-700, -(700*math.tan(math.radians(40)))]
    ...
    def position_cmd(self, raw_pulse, init_pulse, current_deg, target_degree, operating_time, node_id):
        ...
        velocity = int(math.ceil(relative_dist/operating_time))
        accel = int(math.ceil(velocity*2))
        decel = int(math.ceil(velocity*5))
    ...
    def read_position_data(self, data):
        ...
        # check data
        # print('========================')
        # print("dec: ", recv_data_swap)
        # print("hex: ", recv_data_hex)
    ...
    def packet_generator(self, id, data):
        ...
        # packet_str = id_str + data_str + crc_str + term_str
    ...
    def crc16(self, data):
        ...
        for datum in bytearray(data):
            ...
            crc[1] = ncrc >> 8
        # crc16 check
        # print ("Returning an array [%s] %2x %2x" %(', '.join(str(x) for x in crc), crc[0], crc[1]))
        return crc
    ...
    def check_ready(self, node_id):
        if node_id == 0:
            result = []
            for i in [4,5,6]:
                status_dict = self.read_status(i)
    ...
    def scara_warning(self, node_id):
        packet = self.packet_generator(node_id, self.R_WARN)
        self.port.write(packet)
        data_raw = self.check_recv(8)
        data = binascii.hexlify(data_raw)[8:-4]
        if data in self.warningTable.keys():
            return self.warningTable[data]
        else:
            return "Warning code error"
    ...
    def read_status(self, node_id):
        ...
        # Full_status
        # status_dict = {'brake':data_bin_list[0], 'alarm':data_bin_list[1], 'ready':data_bin_list[2], 'zspd':data_bin_list[3],
        #             'inpos1':data_bin_list[4], 'tlmt':data_bin_list[5], 'vlmt':data_bin_list[6], 'inspd':data_bin_list[7],
        #             'warn':data_bin_list[8], 'tgon':data_bin_list[9], 'inpos2':data_bin_list[10]}
        return status_dict
...
#             position_cmd, stop_decel_cmd = self.position_cmd(raw_pulse, current_deg, degree, operating_t, node_id)
            try:
            ...
            #print("recv_data:", data)
            t4 = time.time()
            # print("position", t2-t1, t3-t2, t4-t3, t4-t1)
...
def main():
    scara = Scara()
    while True:
        usr_cmd = raw_input("Test cmd: ")
        if usr_cmd == "on":
            node_id = int(raw_input("Node ID: "))
            scara.sv_on(node_id)

        elif usr_cmd == "off":
            node_id = int(raw_input("Node ID: "))
            scara.sv_off(node_id)
            ...
            node_id = int(raw_input("Node ID: "))
            scara.emg_on(node_id)

        elif usr_cmd == "emg_off":
            node_id = int(raw_input("Node ID: "))
            scara.emg_off(node_id)

        elif usr_cmd == "alarm_reset":
            node_id = int(raw_input("Node ID: "))
            scara.alarm_reset(node_id)

        elif usr_cmd == "warn":
            node_id = int(raw_input("Node ID: "))
            warn = scara.scara_warning(node_id)
            print(warn)

        elif usr_cmd == "alarm":
            node_id = int(raw_input("Node ID: "))
            alarm = scara.scara_alarm(node_id)
            print(alarm)

        elif usr_cmd == "read_status":
            status = []
            for i in range(3):
                status.append(scara.read_status(i+1))

            print(status[0])
            print(status[1])
            print(status[2])

        elif usr_cmd == "read_position":
            node_id = int(raw_input("Node ID: "))
            dec_val, hex_val = scara.read_position(node_id)
            print("dec:", dec_val)
            print("hex:", hex_val)
            ...
            # 1 trun = 262144
            node_id = int(raw_input("Node ID: "))
            degree = float(raw_input("Input degree: "))
            operating_time = float(raw_input("Input operating time: "))
            scara.joint_control(node_id, degree, operating_time)

        elif usr_cmd == "test":
            print("test")

        elif usr_cmd == "q" or usr_cmd == "Q":
            break
