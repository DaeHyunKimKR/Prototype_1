...
import rospy
from scara_function1_final_v1 import Scara as scara_func
...
class Scara_Client():
    def __init__(self):
        ...
        self.scara_func.index_setting([1, 2, 3])
        # ===for motion===
        self.x_list = []
        self.y_list = []
        self.p_time_list = []
    ...
    def pub_scara_status(self):
        while True:
            ...
            for i in range(3):
                try:
                    status.append(self.scara_func.read_status(i+1))
                    raw_pulse, _ = self.scara_func.read_position(i+1)
                except:
                ...
                tune_pulse = raw_pulse - self.init_pulse[i]
                    ...
                    if limit_flag:
                        ...
                        if control_flag:
                            for i in node_id:
                                ...
                                current_deg = self.pub_status.current_joint[i-1]
                                self.scara_func.joint_control(raw_pulse, init_pulse, current_deg, i, target_angle[i-1], target_time)
                        ...
                        if control_flag:
                            for i in node_id:
                                ...
                                if type(target_time) is list:
                                    self.scara_func.joint_control(raw_pulse, init_pulse, current_deg, i, target_angle[i-1], target_time[i-1])
                                else:
                                    target_time = recv_cmd.time
                                    self.scara_func.joint_control(raw_pulse, init_pulse, current_deg, i, target_angle[i-1], target_time)
                            self.scara_func.start(0)
                        ...
                        for count in range(recv_cmd.count):
                            ...
                            for i in range(len(self.x_list)):
                                ...
                                if limit_flag:
                                    ...
                                    if control_flag:
                                        for j in node_id:
                                            init_pulse = self.init_pulse[j-1]
                                            raw_pulse = self.pub_status.current_pulse[j-1]
                                            current_deg = self.pub_status.current_joint[j-1]
                                            self.scara_func.joint_control(raw_pulse, init_pulse, current_deg, j, target_angle[j-1], target_time)
                                        self.scara_func.start(0)
                                        self.scara_func.flush()
                                        t_start = time.time()
                                        while True:
                                ...
                                if limit_flag:
                                    if control_flag:
                                        for j in node_id:
                                            ...
                                            print("target_time type", type(target_time))
                                            if type(target_time) is list:
                                                print("time_list")
                                                self.scara_func.joint_control(raw_pulse, init_pulse, current_deg, j, target_angle[j-1], target_time[j-1])
                                            else:
                                                print("time")
                                                self.scara_func.joint_control(raw_pulse, init_pulse, current_deg, j, target_angle[j-1], target_time)
                                        self.scara_func.start(0)
                    ...
                    if recv_cmd.count != 0:
                        ...
                        while True:
                            for i in node_id:
                                for j in range(5):
                                    self.pub_scara_status()
                                if self.check_ready([i]) and operating_flag[i-1]:
                                    if operating_count[i-1] < recv_cmd.count:
                                        target_angle = target_angle_list[i-1][joint_count[i-1]]
                                        if type(self.j_motion_time_list[joint_count[i-1]]) is list:
                                            target_time = self.j_motion_time_list[joint_count[i-1]][i-1]
                                        else:
                                            target_time = self.j_motion_time_list[joint_count[i-1]]

                                        if i == 1:
                                            if abs(target_angle) > 126:
                                                joint_limit_flag[i-1] = False
                                                print("[SCARA::WARN] JOINT LIMIT")
                                            else:
                                                joint_limit_flag[i-1] = True
                                        elif i == 2:
                                        ...
                                        if (joint_limit_flag[i-1] == True) and (self.pub_status.inpos1[i-1] == True):
                                            init_pulse = self.init_pulse[i-1]
                                            raw_pulse = self.pub_status.current_pulse[i-1]
                                            current_deg = self.pub_status.current_joint[i-1]
                                            ...
                                            self.scara_func.flush()
                                            joint_count[i-1] += 1
                                            if joint_count[i-1] == len(target_angle_list[i-1]):
                                                operating_count[i-1] += 1
                                                joint_count[i-1] = 0
                                                if operating_count[i-1] == recv_cmd.count:
                                                    operating_flag[i-1] = False
                                        # init_joint limit flag
                                        joint_limit_flag[i-1] = True
                                        print("operating_count", operating_count)
                                        print("operating flag: ", operating_flag, "len target angle:", len(target_angle_list[0]), "joint count", joint_count, "check ready", self.check_ready([1,2,3])) 
                            for i in node_id:
                                operating_done = operating_done or operating_flag[i-1] 
