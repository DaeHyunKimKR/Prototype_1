...
#define ID                            1
//Parameter ID
#define PID_PNT_VEL_CMD              207 //for dual motor control
#define PID_MAIN_DATA                210
...
{
	double RPM = 0.0;
	RPM = M_s * 60.0 / (_PI_radius * 2.0);
	return RPM;
}
double RPM2MS(double RPM)
{
	double M_s;
	M_s = RPM * 2.0 * PI * radius / 60.0;
	return M_s;
}
...
Cwheel::Cwheel()
{
  ...
  _x = 0.54/2.0;//half distance between wheels
...
void Cwheel::Open_port()
{
  ...
  _port = open("/dev/WHEEL", O_RDWR|O_NOCTTY);
 ...
 // tty.c_lflag  =  ICANON;
  ...
  std::cout << "Wheel connetion Complete!" <<std::endl;
  usleep(20000);
}
...
void Cwheel::velocity_target(double Linear_vel, double Angular_vel)
{
  ...
  else
  {
    ...
    RightVel2 = 2.0 * Linear_vel - LeftVel2;
...
{ 
  ...
  read(_port, _buf, sizeof(_buf));
    ...
    else
    {
      ...
      error_code_wheel = 1302;
    }

    ///////////// cout to check encoder //////////////////////////
    //std::cout<<"LeftRPM : "<<_LeftRPM <<" (Rotate Per Minute)" <<std::endl;
    //std::cout<<"RightRPM : "<<_RightRPM <<" (Rotate Per Minute)" <<std::endl;
...
//std::cout << LeftVel << "  " << RightVel << "  cmd vel joy" <<std::endl;
//std::cout << _LeftRPM << "  " << _RightRPM << "  read vel" << std::endl;
  //////DO NOT TURN//////////////
  //LeftVel = LeftVel*0.3 + _LeftRPM *0.7;
  //RightVel = RightVel*0.3 + _RightRPM *0.7;
  LeftVel = LeftVel*0.7 + _LeftRPM *0.3;
  RightVel = RightVel*0.7 + _RightRPM *0.3;
